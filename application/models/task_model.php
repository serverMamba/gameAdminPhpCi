...
class task_model extends MY_Model {
    ...
    public function _get_map_idx_by_uid($uid) {
     ...
     public function get_task_his($userid,$gameid,$modeid,$mystarttime,$myendtime,$account,$beginindex) {
        ...
        if (!empty($account)) {
         ...
         $where = "where addtime  >= '$mystarttime' and  addtime <= '$myendtime'";
       ...
       if (!empty($userid)) {
        ...
        $idx = $this->_get_map_idx_by_uid($userid);   
        $idx_db = $idx['idx_db']; 
        $idx_tb = $idx['idx_tb']; 
       ...
       $db = $CI->load->database($this->dbconfig, true);
       
       
       if($modeid == "1")
       {
        
       $daname = "CASINOUSERDB_".$idx_db.".CASINOUSERSYSTEMMISSIONSTATUS_".$idx_tb;
       
       $daname1= "CASINOGLOBALINFO.CASINOSYSTEMMISSION";
       
        
       if($gameid != "10000")
       {
          $sql =  "select * from  $daname left join $daname1 on $daname.missionid = $daname1.id $where and gamecode = $gameid   limit $beginindex , 20 ";
          $sql1 =  "select count(*) as count from  $daname  left join $daname1 on $daname.productid = $daname1.id $where and  gamecode = $gameid  ";
       }else{
          $sql =  "select * from  $daname left join $daname1 on $daname.missionid = $daname1.id $where limit $beginindex , 20 ";
          $sql1 =  "select count(*) as count from  $daname  $where ";   
       }
       
       }else{
           
       $daname = "CASINOUSERDB_".$idx_db.".CASINOUSERDAILYMISSIONSTATUS_".$idx_tb;
       $daname1= "CASINOGLOBALINFO.CASINODAILYMISSION";
