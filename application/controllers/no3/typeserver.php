...
class Typeserver extends MY_Controller {
	public function __construct() {
		parent::__construct ( false, false );
		if (! $this->Common_model->isLogin ()) {
			redirect ( 'no3/login' );
		}
		if (! $this->Common_model->isPriv ( 'ywgl_switch' )) {
			redirect ( 'no3/index' );
		}
		
		$this->load->model ( 'diyconfig_mid_model' );
		$this->load->helper ( 'other' );
	}
	public function save_config_data() {
		$ip = $this->input->get_post ( 'ip' );
		$port = $this->input->get_post ( 'port' );
		$ip1 = $this->input->get_post ( 'ip1' );
		$port1 = $this->input->get_post ( 'port1' );
		$enable = $this->input->get_post ( 'enable' );
		
		if (! empty ( $ip ) && ! empty ( $port ) && ! empty ( $ip1 ) && ! empty ( $port1 )) {
			$ret = $this->diyconfig_mid_model->set_gameserver ( $ip, $port, $ip1, $port1, $enable );
	...
	public function index() {
		...
		$this->load->view ( 'no3/typeserverview', $data );
	}
