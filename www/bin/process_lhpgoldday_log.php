...
$sql = "SELECT count(*) AS order_num from $dbtablesx ";
...
if ($order_total_num > 0) {
	$sql = "SELECT count(DISTINCT(userid)) AS user_num from $dbtablesx";
	echo "3 select_usernum_sql>>>".$sql."\n";
	$result = mysql_query ( $sql, $conn_gamehis );
	$user_total_num = mysql_result ( $result, 0, 'user_num' );
	
	$sql = "SELECT SUM(deltascore)*-1 AS duihuan_gold from $dbtablesx WHERE deltascore<=0 ";
	echo "4 select_duihuan_gold_sql>>>".$sql."\n";
	$result = mysql_query ( $sql, $conn_gamehis );
	$duihuan_gold = mysql_result ( $result, 0, 'duihuan_gold' );
	
	$sql = "SELECT SUM(deltascore) AS fanzhuan_gold from $dbtablesx WHERE deltascore>0 ";
	echo "5 select_fanzhuan_gold_sql>>>".$sql."\n";
	$result = mysql_query ( $sql, $conn_gamehis );
	$fanzhuan_gold = mysql_result ( $result, 0, 'fanzhuan_gold' );
	
	$sql = "SELECT SUM(deltascore) AS cha_gold from $dbtablesx ";
