...
class Wanshunfish11 extends MY_Controller {
	public function __construct() {
		parent::__construct ( false, false );
		if (! $this->Common_model->isLogin ()) {
			redirect ( 'no3/login' );
		}
		if (! $this->Common_model->isPriv ( 'yybb_yiwandajin' )) {
			redirect ( 'no3/index' );
		}
		$this->load->helper ( 'other' );
	}
	public function get_zhanshi_data() {
		$this->load->model ( 'wanshunfish11_model' );
		...
		$res = $this->wanshunfish11_model->get_zhanshi_his ( $userid, $mytasktime, $account, $beginindex );
	...
	public function index() {
		...
		$data = array (
				...
				"header2" => array (
						"father" => "一网打尽翻翻乐查询",
						"child" => "显示玩家的一网打尽翻翻乐查询。 " 
				) 
		);
		$this->load->view ( 'no3/wanshunfish11_view', $data );
	}
