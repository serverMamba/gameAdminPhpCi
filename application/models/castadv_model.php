 ...
 * 支付相关
 */
class castadv_model extends MY_Model {
    var $db = null;
    ...
    public function __construct() {
     ...
     public function update_adv_msg($data) {
        ...
        foreach ($data as $index => $row) {
            ...
            $db->where('game_sub_version', $key);
            $ret = $db->update("CASINOGAMEADWALLINFO", $record);
        }
    }

    public function save_adv_msg($status, $version,$systemid,$gameid){
        $CI = &get_instance();
        $db = $CI->load->database('dbhischart', true);  
        ...
        $data = array(
            "adwall_code"=>$gameid,
            "gamecode"=>$gameid,
            "game_root_version"=>$systemid,
            "game_sub_version"=>$version,
            "adwallorder"=>$status
        );
        $db->insert("CASINOGAMEADWALLINFO", $data);
       // $db->trans_commit();
        return $db->affected_rows(); 
        
    }
    
    public function get_adv_msg($action,$mainversionid,$subversionid,$systemid,$gameid,$beginindex){
        $CI = &get_instance();
        $db = $CI->load->database('dbhischart', true);
        ...
        if($systemid !="9999"){
          $wherelist[] = "game_root_version = $systemid" ; 
        }
        
        if($gameid !="9999"){
          $wherelist[] = "gamecode = $gameid"  ;
        }
        
        if(!empty($subversionid)){
          $wherelist[] = "game_sub_version = $subversionid"  ;
        }
