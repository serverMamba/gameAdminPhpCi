...
class Wanshunfish5_model extends MY_Model {
    ...
    public function get_exchange_his($userid, $mystarttime, $myendtime, $beginindex){
        ...
        $dbtablesx = "CASINOITEMCHANGEHISTORY";
      ...
      //  for($ii = $startx ;$ii<= $endx;$ii=$ii+60*60*24){
            $ii = $startx;
             $tablename = $dbtablesx.date('Ym', $ii);
             ...
             $tableflag1 = count($retzz);
              ...
              if($tableflag1>0){
                ...
                if(strlen($sqltable)>2){
                    $sqltable =  $sqltable. " union all (select * from  $tablename)";  
                }else{
                    $sqltable =  $sqltable. "(select * from  $tablename)";
                }
                   * 
                   */
       ...
       //  $sqltable =  $sqltable. ") as aa";
         ...
         $where="";
        if(strlen($userid) !=0)   {$xx = strlen($where)>0? " and ":"" ; $where = $where.$xx." userid = '$userid'";}
        if(strlen($mystarttime) !=0)  {$xx = strlen($where)>0? " and ":"" ; $where = $where.$xx." changetime >= '$mystarttime'";}
        if(strlen($myendtime) !=0)  {$xx = strlen($where)>0? " and ":"" ; $where = $where.$xx." changetime <= '$myendtime'";}
      //  if(strlen($paijuhao) !=0)  {$xx = strlen($where)>0? " and ":"" ; $where = $where.$xx." aa.game_number = '$paijuhao'";}
        
         $sql = "$sqltable order by recordtime desc limit $beginindex , 20 ";
         
       
        $sql1 = "select count(*) as count from  $tablename";
        
        if(strlen($where)>0)
        {
           $sql =  "$sqltable where $where order by changetime desc limit $beginindex ,20 "; 
           $sql1 = "select count(*) as count from  $tablename where $where"; 
        }
