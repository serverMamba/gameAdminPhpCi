...
//            $finalRet = $this->dindan_model->getOrderListByTypeTwo();
        ...
        $url = site_url('no3/homePage/index') . '?' . http_build_query($query);
        $config ['base_url'] = $url;
    ...
    public function getDetail() {
        ...
        $dateTimeBegin = date('Y-m-d 00:00:00', $ts);
        $dateTimeEnd = date('Y-m-d 23:59:59', $ts);

        $finalRet = [];
        $ret = [];
        $this->load->model('home_model');
        $ret = $this->home_model->getDetail($dateTimeBegin, $dateTimeEnd);
