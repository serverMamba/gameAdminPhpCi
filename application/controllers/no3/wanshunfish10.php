...
class Wanshunfish10 extends MY_Controller {
	public function __construct() {
		parent::__construct ( false, false );
		if (! $this->Common_model->isLogin ()) {
			redirect ( 'no3/login' );
		}
		if (! $this->Common_model->isPriv ( 'yybb_puyurenwuchaxun' )) {
			redirect ( 'no3/index' );
		}
		$this->load->helper ( 'other' );
	}
	public function get_zhanshi_data() {
		$this->load->model ( 'wanshunfish10_model' );
		...
		$res = $this->wanshunfish10_model->get_zhanshi_his ( $userid, $mytasktime, $account, $beginindex );
	...
	public function index() {
		...
		$this->load->view ( 'no3/wanshunfish10_view', $data );
	}
