...
while ( true ) {
	...
	if (mysql_num_rows ( $query ) > 0) {
		while ( $row = mysql_fetch_array ( $query, MYSQL_ASSOC ) ) {
			...
			$sql_cash = "select IFNULL(SUM(cash_money),0) AS cash_money FROM db_smc.smc_cash_order WHERE user_id = '$user_id' AND status = 1";
			...
			if ($db_index <= 7) {
				$query222 = mysql_query ( $sql_cash, $conn_u1 );
				$query333 = mysql_query ( $sql_user1, $conn_u1 );
			} else {
				$query222 = mysql_query ( $sql_cash, $conn );
				$query333 = mysql_query ( $sql_user1, $conn );
			}
			
			$db_cash_money = mysql_result ( $query222, 0, 'cash_money' );
			$total_cash_money = mysql_result ( $query333, 0, 'total_total_money' );
			
			if ($db_cash_money != $total_cash_money) {
				$totalBuy = mysql_result ( $query333, 0, 'totalBuy' );
				$last_login_time = mysql_result ( $query333, 0, 'last_login_time' );
				$lastLoginIp = mysql_result ( $query333, 0, 'lastLoginIp' );
				
				$sql_in = "INSERT INTO db_smc.smc_tmp1 (user_id,total_pay,total_cash_money,db_cash_money,last_login_time,last_login_ip) VALUES
											('$user_id','$totalBuy','$total_cash_money','$db_cash_money','$last_login_time','$lastLoginIp')";
