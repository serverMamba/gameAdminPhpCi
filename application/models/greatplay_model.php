 ...
 * 支付相关
 */
class greatplay_model extends MY_Model {
    var $db = null;
    ...
    public function __construct() {
     ...
     public function get_greatplay_his($statusid,$mystarttime,$myendtime,$beginindex) {
       ...
       $sqllistx = implode(" union all ",$sqllist);
        ...
        $sql = "select aa.userid as userid ,count(aa.userid) as feecount,sum(aa.totalfee) as feesum,min(aa.totalfee) as feemin,max(aa.totalfee) as feemax,max(tradeTime) as timemax, min(tradeTime) as timemin ,count(aa.userid)*3600*24/(max(tradeTime) - min(tradeTime)) as dettime  from $sqltable  group by aa.userid  ";
        
        if($statusid =="1"){
           $sqlx = "select * from ($sql) as cc order by cc.feesum desc limit $beginindex , 20"; 
        }
        
       if($statusid =="2"){
           $sqlx = "select * from ($sql) as cc order by cc.feecount desc limit $beginindex , 20"; 
        }
        
       if($statusid =="3"){
           $sqlx = "select * from ($sql) as cc order by cc.dettime desc limit $beginindex , 20"; 
        }
