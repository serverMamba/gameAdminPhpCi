...
class Wanshunfish2dw_model extends MY_Model {
    ...
    public function get_exchange_his($userid, $mystarttime, $myendtime, $beginindex){
        ...
        $dbtablesx = "CASINOFRUITMACHINECOMPAREGAMERECORD";
        ...
        for($ii = $startx ;$ii<= $endx;$ii=$ii+60*60*24){
         ...
         $where="";
        if(strlen($userid) !=0)   {$xx = strlen($where)>0? " and ":"" ; $where = $where.$xx." aa.userid = '$userid'";}
        if(strlen($mystarttime) !=0)  {$xx = strlen($where)>0? " and ":"" ; $where = $where.$xx." aa.recordtime >= '$mystarttime'";}
        if(strlen($myendtime) !=0)  {$xx = strlen($where)>0? " and ":"" ; $where = $where.$xx." aa.recordtime <= '$myendtime'";}
      //  if(strlen($paijuhao) !=0)  {$xx = strlen($where)>0? " and ":"" ; $where = $where.$xx." aa.game_number = '$paijuhao'";}
        
         $sql = "select * from $sqltable order by aa.recordtime desc limit $beginindex , 20 ";
        ...
        if(strlen($where)>0)
        {
           $sql =  "select * from $sqltable where $where order by aa.recordtime desc limit $beginindex ,20 "; 
           $sql1 = "select count(*) as count from $sqltable where $where"; 
        }
