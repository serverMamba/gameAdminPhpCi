...
class Wanshun4_model extends MY_Model {
    ...
    public function get_exchange_his($gametype,$mydate,$iswin,$wintype){
        $CI = &get_instance();
        $db = $CI->load->database('globalinfo', true);

        if($iswin == "0"){
            $where = "where statistics_date  = '$mydate'  and wintype = $wintype and iswin = $iswin and gametype = $gametype order by winscore ASC"; 
        }else{
            $where = "where statistics_date  = '$mydate'  and wintype = $wintype and iswin = $iswin and gametype = $gametype order by winscore DESC"; 
        }

        $sql =  "select * from  CASINOGLOBALINFO.CASINODETAILTOPWINSCOREUSER $where  ";
