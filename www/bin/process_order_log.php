...
$sql = "DELETE FROM smc_log_order WHERE date = '$date_db'";
mysql_query ( $sql, $conn );

foreach ( $channellist as $k => $v ) {
	$sql = "SELECT id FROM smc_log_order WHERE date = '$date_db' AND channel_id = '$k' LIMIT 1";
	$result = mysql_query ( $sql, $conn );
	if (mysql_num_rows ( $result ) > 0) {
		exit ( 'log is exist' . $line );
	}
	
	$sql = "SELECT count(*) AS order_num from smc_order WHERE `status` = 1 AND channel_id = '$k' AND add_time >= '$db_time_start' AND add_time < '$db_time_end'";
	$result = mysql_query ( $sql, $conn );
	$order_total_num = mysql_result ( $result, 0, 'order_num' );
	
	if ($order_total_num > 0) {
		$sql = "SELECT count(DISTINCT(user_id)) AS user_num from smc_order WHERE `status` = 1 AND channel_id = '$k' AND add_time >= '$db_time_start' AND add_time < '$db_time_end'";
		$result = mysql_query ( $sql, $conn );
		$user_total_num = mysql_result ( $result, 0, 'user_num' );
		
		$sql = "SELECT SUM(money) AS total_money from smc_order WHERE `status` = 1 AND channel_id = '$k' AND add_time >= '$db_time_start' AND add_time < '$db_time_end'";
		$result = mysql_query ( $sql, $conn );
	...
	$sql = "INSERT INTO smc_log_order (date,order_total_num,user_total_num,pay_total_num,cash_money,choushui_money,cash_total_money,channel_id,date1)
	VALUE ('$date_db','$order_total_num','$user_total_num','$pay_total_num','$cash_money','$choushui_money','$cash_total_money','$k','$date1')";
	mysql_query ( $sql, $conn );
}

$sql = "SELECT id FROM smc_log_order WHERE date = '$date_db' AND channel_id = 0 LIMIT 1";
$result = mysql_query ( $sql, $conn );
if (mysql_num_rows ( $result ) > 0) {
	exit ( 'log is exist' . $line );
}
...
$sql = "SELECT count(*) AS order_num from smc_order WHERE `status` = 1 AND add_time >= '$db_time_start' AND add_time < '$db_time_end' $where";
$result = mysql_query ( $sql, $conn );
$order_total_num = mysql_result ( $result, 0, 'order_num' );

if ($order_total_num > 0) {
	$sql = "SELECT count(DISTINCT(user_id)) AS user_num from smc_order WHERE `status` = 1 AND add_time >= '$db_time_start' AND add_time < '$db_time_end' $where";
	$result = mysql_query ( $sql, $conn );
	$user_total_num = mysql_result ( $result, 0, 'user_num' );
	
	$sql = "SELECT SUM(money) AS total_money from smc_order WHERE `status` = 1 AND add_time >= '$db_time_start' AND add_time < '$db_time_end' $where";
	echo $sql."\n";
	$result = mysql_query ( $sql, $conn );
	$pay_total_num = mysql_result ( $result, 0, 'total_money' );
} else {
