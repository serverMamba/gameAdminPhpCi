...
class Reporthisfish extends CI_Controller {
	public function __construct() {
		parent::__construct ( false, false );
		if (! $this->Common_model->isLogin ()) {
			redirect ( 'no3/login' );
		}
		if (! $this->Common_model->isPriv ( 'yybb_bulizxs' )) {
			redirect ( 'no3/index' );
		}
		$this->load->model ( 'no3/Admin_model' );
	}
	public function get_hisfish_data() {
		$this->load->model ( 'hisfish_mid_model' );
		...
		$res = $this->hisfish_mid_model->get_hismsg1 ( $gameid, $mytime );
	...
	public function index() {
		...
		$gamecodehuang = $this->config->item ( 'gamefishlist' );
		...
		$this->load->view ( 'no3/reporthisfishview', $data );
	}
