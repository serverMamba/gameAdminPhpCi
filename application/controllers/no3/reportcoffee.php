...
class Reportcoffee extends MY_Controller {
	public function __construct() {
		parent::__construct ( false, false );
		if (! $this->Common_model->isLogin ()) {
			redirect ( 'no3/login' );
		}
		if (! $this->Common_model->isPriv ( 'yybb_coffee' )) {
			redirect ( 'no3/index' );
		}
	}
	public function get_his_data() {
		$this->load->model ( 'coffee_mid_model' );
		$gameid = $this->input->get_post ( 'gameid' );
		$mytime = $this->input->get_post ( 'mytime' );
		$res = $this->coffee_mid_model->get_hismsg ( $gameid, $mytime );
	...
	public function index() {
		...
		$data = array (
				...
				"header2" => array (
						"father" => "金豆和保险箱变化表【金豆单位：元】",
						"child" => "用曲线方式显示金豆和保险箱变化表在线人数" 
				)
		);
		$this->load->view ( 'no3/reportcoffeeview', $data );
	}
