    ...
    <body>
       ...
       <div id="modal-tree-items" class="modal" tabindex="-1">
        ...
        <tr >
            ...
            <td class="left" >${gamename}</td>
            <td class="left" >${packagename}</td>
            <td class="left" >${version}</td>
            <td class="left" >${overversion}</td>
            <td class="left" id="myTag${gamename}">${url}</td>
   
            <td class="hidden-480"><input type="text" id="version${serveripcc}" value="${version}" placeholder="最新版本号修改" class="col-xs-20 col-sm-10" style="margin-left: 10px;width:80px;" /></td>
             <td class="hidden-480"><input type="text" id="overversion${serveripcc}" value="${overversion}" placeholder="过期版本号修改" class="col-xs-20 col-sm-10" style="margin-left: 10px;width:80px;" /></td>
             <td class="hidden-480"><input type="text" id="url${serveripcc}"  value="${url}" placeholder="过期版本号修改" class="col-xs-20 col-sm-10" style="margin-left: 10px;width:360px;" /></td>
            <td width ="100" ><button onclick="javascript:updateconfig('${packagename}','${serveripcc}')" class="btn btn-xs btn-success " style="margin-top:1px;">
                                ...
                                <div class="col-xs-12 col-sm-12 widget-container-span">
                                    <div class="widget-box">
                                        <div class="widget-header header-color-blue2">
                                            <h5><i class="icon-arrow-left"></i>IOS升级服务器管理</h5>
                                                ...
                                                <table id="sample-table-2" class="table table-striped table-bordered table-hover">
                                                    <thead id="targethead">
                                                        <tr>
                                                          <!--  <th>渠道名称</th>
                                                            <th>签名</th>  -->
                                                             <th style="width:200px">IOS游戏中文名称</th>
                                                              <th style="width:100px">包名</th>
                                                             <th style="width:80px">最新版本</th>
                                                             <th style="width:80px" >过期版本</th>
                                                              <th style="width:250px">IOS的升级地址</th>
                                                             
                                                             <th style="width:80px">最新版本修改</th>
                                                             <th style="width:80px">过期版本修改</th>
                                                              <th style="width:250px">IOS的升级地址修改</th>
                                                            <th>实际操作</th>
                                                          </tr>
                                                    </thead>
        ...
        function get_online_data() {
            ...
            function onsuccess(data) {
                ...
                for (var itemx in datax){
                     var ip = datax[itemx]["packagename"];
                     if(ip != null && ip.length>0){
                      ip = ip.split(".").join('_'); 
                      datax[itemx]["serveripcc"] =  ip;
                     }
                  }
            ...
            jQuery.comm.sendmessage(window.location.protocol + "//" + window.location.host + "/no3/packetupgradeios/get_proxy_data", packet, onsuccess, onerrors);
        }
        
        
        function updateconfig(packagename,serveripcc){
             var version =$("#version"+serveripcc).val();
             var overversion =$("#overversion"+serveripcc).val();
             var url =$("#url"+serveripcc).val();
            ...
            jQuery.comm.sendmessage(window.location.protocol + "//" + window.location.host + "/no3/packetupgradeios/update_proxy_data", packet, onsuccess, onerrors);
        }
