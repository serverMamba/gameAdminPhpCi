...
<html lang="en">
    ...
    <body>
        ...
        <div id="modal-tree-items" class="modal" tabindex="-1">
         ...
         <div id="modal-tree-genkami" class="modal" tabindex="-1">
            <div class="modal-dialog" style="width:550px;margin-top: 100px;">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                        <h4 class="blue bigger">卡密生成</h4>
                    </div>
                    ...
                    <form class="form-horizontal" role="form">
                            ...
                            <div class="form-group">
                                <label class="col-sm-3 control-label no-padding-right" for="form-field-y"> 价格： </label>
                                <div class="col-sm-9">
                                    <input type="text" id="form-field-price" placeholder="数值" value="50" class="col-xs-20 col-sm-10" style="margin-left: 10px;width:230px;" />
                                </div>
                            </div>
                            
                             <div class="space-4"></div>
                            <div class="form-group">
                                <label class="col-sm-3 control-label no-padding-right" for="form-field-y"> 过期时间： </label>
                                <div class="col-sm-9">
                                    <input type="text" id="form-field-exp" placeholder="数值" value="360" class="col-xs-20 col-sm-10" style="margin-left: 10px;width:230px;" />
                                </div>
                            </div>
 
                            <div class="space-4"></div>
                            <div class="form-group">
                                <label class="col-sm-3 control-label no-padding-right" for="form-field-m"> 数量： </label>
                                <div class="col-sm-9">
                                    <input type="text" id="form-field-count" placeholder="数值" value="1000" class="col-xs-20 col-sm-10" style="margin-left: 10px;width:230px;" />
                                </div>
                            </div>
                    ...
                    <div class="modal-footer">
                        <button class="btn btn-sm" data-dismiss="modal"><i class="ace-icon fa fa-times"></i> Cancel</button>
                        <button class="btn btn-sm btn-primary" onclick="generatedo();"><i class="ace-icon fa fa-check"></i> 生成卡密</button>
                    </div>
...
<script id="datamodel1" type="text/html" >
    <tr>
        <td>${id}</td>
        <td ><i class="icon-time bigger-110 hidden-480"></i>${createtime}</td>
        <td>${expdate}</td>
        <td>${money}</td>
        <td>${realmoney}</td>
        <td>${mac}</td>
                            ...
                            <div class="col-xs-12 col-sm-12 widget-container-span">
                                <div class="widget-box">
                                    <div class="widget-header header-color-blue2">
                                        <h5><i class="icon-arrow-left"></i>卡密管理系统</h5>
                                    ...
                                    <div class="widget-toolbox padding-8 clearfix">
                                        ...
                                        <button onclick="javascript:reflesh()" class="btn btn-xs btn-success " style="margin-top:1px;">
                                          ...
                                          <button onclick="javascript:generate()" class="btn btn-xs btn-success " style="margin-top:1px;">
                                            <span class="bigger-110">卡密生成</span>

                                            <i class="icon-camera icon-on-right"></i>
                                        </button>
                                        ...
                                        <button onclick="javascript:commitphone()" class="btn btn-xs btn-success " style="margin-top:1px;">
                                            <span class="bigger-110">卡密审核</span>
...
<?php } ?>
    ...
    function generate(){
        $('#modal-tree-genkami').modal('show');   
    }
    
    function generatedo(){
         var price = $("#form-field-price").val();
         var exp = $("#form-field-exp").val();
         var count = $("#form-field-count").val();
        var packet = {
            action: 'generatedo',
            price: price,
            exp: exp,
        ...
        function onsuccess(data) {
              $('#modal-tree-genkami').modal('hide');   
        }
        function onerrors(data) {
            // alert(objtostr(data))
        }
        jQuery.comm.sendmessage(window.location.protocol + "//" + window.location.host + "/no3/castkami/generatedo", packet, onsuccess, onerrors);
     }
    ...
    function get_online_data() {
        ...
        jQuery.comm.sendmessage(window.location.protocol + "//" + window.location.host + "/no3/castkami/get_kami_data", packet, onsuccess, onerrors);
    }
