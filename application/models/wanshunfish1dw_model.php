...
class Wanshunfish1dw_model extends MY_Model {
    ...
    public function get_exchange_his($userid, $mystarttime, $myendtime, $beginindex){
        ...
        $dbtablesx = "CASINOFRUITMACHINENORMALGAMERECORD";
        ...
        if(strlen($mystarttime) !=0)  {$xx = strlen($where)>0? " and ":"" ; $where = $where.$xx." recordtime >= '$mystarttime'";}
        if(strlen($myendtime) !=0)  {$xx = strlen($where)>0? " and ":"" ; $where = $where.$xx." recordtime <= '$myendtime'";}
