...
class Report extends CI_Controller {
	...
	var $availableDevices = array(
				//'16C89E03-4579-4E7F-955F-7E8BEA231C74', //pang zi
				'8979BF65-CD8E-438B-BF7C-ED5ED2BD29B2', //gangshan test machine
                'C59A52E3-1924-463F-BE0E-8706C135620B',  //coffee
				'CF9CD5B3-F69D-464A-B569-7491DB361F7D',	//pangzi
				'6F29AAE5-1F14-49F0-A1B1-91ABEB568B05',//coffee2
			);
	...
	var $agents = array(
		'my' => array(
			'channels' => array(
				...
				91010 => '开心赢三张Android(广州波哥)',
				91011 => '肥猫斗地主Android(广州波哥)',
				91012 => '肥猫斗地主Android(广州波哥)',
	...
	public function __construct() {
		...
		//return;
		// [170326] 先检测userAgent对不对
		$userAgent = $_SERVER['HTTP_USER_AGENT'];
		// $userAgent = 'Mozilla/5.0 (iPhone; CPU iPhone OS 9_1 like Mac OS X) AppleWebKit/601.1.46 (KHTML, like Gecko) Version/9.0 Mobile/13B143 Safari/601.1t SMC/1.0(111111_1490533077_f52e1740a51d13602dadf80c01aa1903)';
		$this->writeLog($userAgent);
		
		$matches = array();
		$matchRet = preg_match('/SMC\/1\.0\((.*)\)/', $userAgent, $matches);
		if ($matchRet == 1)
		{
			$extractedString = $matches[1];
			$exploded = explode('_', $extractedString);
			$deviceID = $exploded[0];
			$timestamp = intval($exploded[1]);
			$sign = $exploded[2];
			...
			{
				// 时间不正确
				$this->writeLog("Out of garanteed time, match result: " . $extractedString);

				$data['errMsg'] = "不在有效时间内，请重新登录";
				echo ($this->load->view ( 'report/error_view', $data, true));
				exit();
			}
			$this->writeLog($deviceID);
			// 2. 判断设备是否在列表中
			if (!in_array($deviceID, $this->availableDevices))
			{
				$this->writeLog("Not in availableDevices, match result: " . $extractedString);
			...
			// 3. 判断sign
			$calcSign = md5($deviceID . "&" . $timestamp . "&" . $this->VERIFY_KEY);
			if ($calcSign != $sign)
			{
				$this->writeLog("Sign error!!! match result: " . $extractedString . ", calcSign: " . $calcSign);
		...
		{
			// 验证失败
			$this->writeLog("USER_AGENT error!!!" . $userAgent);
	...
	private function writeLog($txt) {
		$log_file = "/log/report_app.log";
		$handle = fopen ( $log_file, "a+" );
		$dateTime = date("Y-m-d H:i:s", time());
		$ip = $this->getIp();
		$str = fwrite ( $handle, "[$dateTime][$ip] " . $txt . "\n" );
		fclose ( $handle );
	}
	...
	private function getValueByDateChannelKey($time, $channelId, $key)
	{
		...
		if (false&&file_exists ( $dir . '/' . $myfilename )) {
			...
			$result = $this->Agent_model->get_total_static ( $time, $key, $channelId );
			if (! is_dir ( $dir )) {
				mkdir ( $dir );
			}
			$today = date ( 'Y-m-d' );
	...
	private function getValueOfLastMonth($channelId, $key)
	{
		...
		$cur_month = "";
		$time = date ( 'Y-m-d' );
		$this->Agent_model->getLastCurMonth($time, $last_month, $cur_month);
		...
		if (file_exists ( $dir . '/' . $myfilename )) {
			...
			$result = $this->Agent_model->get_total_static ( $time, $key . '_last_month', $channelId );
			if (! is_dir ( $dir )) {
				mkdir ( $dir );
			}
			if (date ( 'd' ) != '01') {
	...
	public function tongji() {
		...
		foreach ( $field_priv as $t ) {
			$data ['cur_month_total_data'][$t] = $this->Agent_model->get_total_static ( $time1, $t . '_cur_month', $channel_id );
		}
	...
	private function specAgentList($agentName)
	{
		...
		for($i = $end_time; $i >= $start_time; $i -= 3600 * 24) {
			...
			foreach ( $field_priv as $t ) {
				...
				{
					$report_data [$t] = $this->Agent_model->get_total_static ( $time, $t, $channel_id, $channel_list );
				}
				else 
				{
					$report_data [$t] = $this->getValueByDateChannelKey($time, $channel_id, $t);
		...
		foreach ( $field_priv as $t ) {
			...
			if ($channel_id == $this->ALL_CHANNEL_ID)
			{
				$data ['last_month_total_data'][$t] = $this->Agent_model->get_total_static ( $time, $t . '_last_month', $channel_id, $channel_list );
			}
			else 
			{
				$data ['last_month_total_data'][$t] = $this->getValueOfLastMonth($channel_id, $t);
		...
		$data ['cur_month_total_data'] = array ();
		foreach ( $field_priv as $t ) {
			$data ['cur_month_total_data'] [$t] = $this->Agent_model->get_total_static ( $time, $t . '_cur_month', $channel_id, $channel_list);
		}
	...
	public function bossZhouReport()
	{
		...
		$data ['fin_list'] = $this->Agent_model->getMyChannelTotal ( $query ['start_date'], $query ['end_date'], $this->agents['bossZhou']['channels'] );
	...
	public function channelReport() {
		...
		$data ['fin_list'] = $this->Agent_model->getTotal ( $query ['start_date'], $query ['end_date'] );
	...
	public function myChannelReport() {
		...
		$data ['fin_list'] = $this->Agent_model->getMyChannelTotal ( $query ['start_date'], $query ['end_date'], $this->agents['my']['channels'] );
		// $data ['total'] = $data ['fin_list'] ['全部'];
		// unset ( $data ['fin_list'] ['全部'] );
	...
	public function rechargeReport() {
		...
		$data ['report_data'] = $this->Agent_model->getOrderStatistics ( $query ['channel_id'], date ( 'Ymd' ) );
	...
	public function rechargePlatform() {
		...
		$data ['report_data'] = $this->Agent_model->getPayStatics ( date ( 'Y-m-d' ), date ( 'Y-m-d' ) );
