(function(){
        ...
        H5lock.prototype.storePass = function(psw) {// touchend结束之后对密码和状态的处理
            if (this.pswObj.step == 1) {
                ...
                if (this.checkPass(this.pswObj.spassword, psw)) {
                    document.getElementById('title').innerHTML = '解锁成功,请稍后,跳转中...';
                    this.drawStatusPoint('#2CFF26');
                    $('#bg').show();
                    location.href = base_url + 'report/tongji?username='+window.localStorage.getItem('username')+'&password='+window.localStorage.getItem('password');
                } else {
                    this.drawStatusPoint('red');
        ...
        H5lock.prototype.makeState = function() {
            if (this.pswObj.step == 2) {
                ...
                document.getElementById('title').innerHTML = '请输入密码';
        ...
        H5lock.prototype.initDom = function(){
            ...
            var str = '<h5 id="title" class="title">绘制解锁图案</h5>'+
                      '<a id="updatePassword" style="position: absolute;right: 5px;top: 5px;color:#fff;font-size: 10px;display:none;">重置密码</a>'+
                      '<canvas id="canvas" width="250" height="250" style="background-color: #305066;display: inline-block;"></canvas>';
        ...
        H5lock.prototype.bindEvent = function() {
            ...
            this.canvas.addEventListener("touchstart", function (e) {
             ...
             document.getElementById('updatePassword').addEventListener('click', function(){
                 //self.updatePassword();
            	 location.href = base_url + 'report/toLogin';
             });
        }
})();
