...
$sql = "DELETE FROM smc_log_lhpgolduser WHERE date = '$date_db'";
echo "1-1>>> ".$sql."\n";
...
$sql = "SELECT id FROM smc_log_lhpgolduser WHERE date = '$date_db' LIMIT 1";
...
while ( $rowTabItem = mysql_fetch_array ( $result ) ) {
	...
	$sql = "INSERT INTO smc_log_lhpgolduser (date,userid,duihuan_gold,fanzhuan_gold,cha_gold,date1)
	VALUE ('$date_db','$userid','$duihuan_gold','$fanzhuan_gold','$cha_gold','$date1')";
	$resultInsert = mysql_query ( $sql, $conn_default );
	echo "7 ".++$num.",$resultInsert insert_sql>>>".$sql."\n";
	...
	$resultInsert = mysql_query ( $sql, $conn_default );
	echo "8 ".$num.",$resultInsert insert_sql>>>".$sql."\n";
}
