...
<html lang="en">
    ...
    <script id="datamodel1" type="text/html" >
        <tr>
             <td><i class="icon-time bigger-110 hidden-480"></i>${tradeTime}</td>
            <td class="center">${userid}</td>
          
            <td class="hidden-480">${gamecode}</td>
            <td>${tradeno}</td>
            <td class="hidden-480">${ platformid}</td>
            <td>${channelid}</td>
            <td class="hidden-480">${totalfee}</td>
            <td>${productid}</td>
                                ...
                                <div class="col-xs-12 col-sm-12 widget-container-span">
                                    <div class="widget-box">
                                        <div class="widget-header header-color-blue2">
                                            <h5><i class="icon-arrow-left"></i>玩家商城记录</h5>
                                        ...
                                        <div class="widget-toolbox padding-8 clearfix">
                                            ...
                                            <div class="widget-toolbar no-border pull-left" style="width:150px;height:32px;"  >
                                                  <select class="zsh-select form-control"  id="form-field-select-3" data-placeholder="Choose a game..." style="height:32px;width:130px;">
                                                     <option value="9999">全部（9999）</option>
                                                  ...
                                                  <select class="zsh-select form-control"  id="form-field-select-4" data-placeholder="Choose a game..." style="height:32px;width:130px;">
                                                   <option value="9999">全部（9999）</option>
                                                    <?php foreach ($paylist as $myid => $myname) { ?>
                                                     <option value="<?php echo $myid ?>"><?php echo $myname["name"] ?></option>
                                                    <?php } ?> 
                                                  </select>
                                            </div>
                                          
                                            <input type="text" id="userid1"   placeholder="用户ID"   class="col-xs-10 col-sm-2" style = "margin-left:0px;height:30px;width:120px;"/>
                                            <input type="text" id="orderid1"   placeholder="订单号"   class="col-xs-10 col-sm-2" style = "margin-left:0px;height:30px;width:120px;"/>
    ...
    <script type="text/javascript">
        ...
        producttype1[2] = "兑换道具";
         ...
         var channelx = new Array();
         <?php foreach ($channellist as $myid => $myname) { ?>
             channelx["<?php echo $myid ?>"] = "<?php echo $myname ?>";
        <?php } ?>
        ...
        function get_online_data() {
            ...
            var packet = {
                ...
                platformid: $("#form-field-select-4").val(),
            ...
            function onsuccess(data) {
                ...
                for (var itemx in datax["detail"])
                {
                   ...
                   datax["detail"][itemx]["channelid"]  = channelx[datax["detail"][itemx]["channelid"]];
                    datax["detail"][itemx]["platformid"]  = paylistx[getfixstr(datax["detail"][itemx]["platformid"],4)];
                    datax["detail"][itemx]["gamecode"]  =  gamelistx[getfixstr(datax["detail"][itemx]["gamecode"],3)];
                 }
                 
                  var sumx =datax["sum"];
                var msg = "总共数据：" + allcount + "条,现在是在第：" + beginindex + "页，每页20条,金额合计为："+sumx+"元";
            ...
            jQuery.comm.sendmessage(window.location.protocol + "//" + window.location.host + "/no3/infoshop/get_shop_data", packet, onsuccess, onerrors);
        }
        ...
        function getExcel(){
             ...
             var param = "infoshop/get_shopexcel_data?" +
                "mystarttime=" + $("#id_date_picker_1").val() + " " + $("#id_time_picker_1").val() +
                "&myendtime=" + $("#id_date_picker_2").val() + " " + $("#id_time_picker_2").val() +
                "&userid=" + $("#userid1").val() +
                "&gameid=" + $("#form-field-select-3").val() +
