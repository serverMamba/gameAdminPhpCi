...
writeLog("switchVIP3Redis start --------------");
...
function switchRedisProxys ($config){
	if($config){
		...
		$redis->set ( 'vip3_iplist', json_encode($ip_list) );
		$redis->close ();
		$num = count($ip_list);
		writeLog("switchVIP3Redis success $num:".json_encode($ip_list));
		return $num;
	}else{
		writeLog("switchVIP3Redis error: config=$config");
	}
	return 0;
}
