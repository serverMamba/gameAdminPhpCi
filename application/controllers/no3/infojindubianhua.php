...
class Infojindubianhua extends MY_Controller {
	public function __construct() {
		parent::__construct ( false, false );
	if (! $this->Common_model->isLogin ()) {
			redirect ( 'no3/login' );
		}
		if (! $this->Common_model->isPriv ( 'kfgl_wjjdbh' )) {
			redirect ( 'no3/index' );
		}
		$this->load->helper ( 'other' );
	}
	public function get_jindubianhua_data() {
		$this->load->model ( 'jindubianhua_model' );
		...
		/*
		 * if (empty($account)&&empty($userid)) { echo
		 * json_encode(array("status"=>"1")); return; }
		 */
		
		$res = $this->jindubianhua_model->get_jindubianhua_his ( $userid, $eventid, $gameid, $mystarttime, $myendtime, $account, $beginindex );
		
		echo json_encode ( $res );
	}
	public function get_jindubianhua_data_excel() {
		$this->load->model ( 'jindubianhua_model' );
		...
		$res = $this->jindubianhua_model->get_jindubianhua_his_excel ( $userid, $eventid, $gameid, $mystarttime, $myendtime, $account, $beginindex );
	...
	public function index() {
		...
		$data = array (
				...
				"gamelists" => $gamecodehuang,
				"eventlists" => $this->config->item ( 'eventlists' ),
				"choose" => array (
						"father" => "客服管理",
						"child" => "玩家金豆变化记录" 
				),
		...
		$this->load->view ( 'no3/infojindubianhuaview', $data );
	}
