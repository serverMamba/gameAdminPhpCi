...
<body style="background:url('<?php echo $base_url; ?>images/ppp.jpg') no-repeat;">
	...
	<select id="report" onchange="select_report();">
		...
		<option value="8">关哥的渠道</option>
		<option value="6">我的渠道</option>
		<option value="7">我的渠道排行</option>
		<option value="9">周老板的渠道</option>
		<option value="10">周老板渠道排行</option>
	</select>
	<?php } ?>
	...
	$(function() {
		...
		function select_report(){
			...
			if(c_id == 1){
				...
				channelTonji('<?php echo $base_url.'report/myChannelReport'; ?>');
			}else if(c_id == 8){
				channelTonji('<?php echo $base_url.'report/guangeTongjiList'; ?>');
			}else if(c_id == 9){
				channelTonji('<?php echo $base_url.'report/bossZhouTongjiList'; ?>');
			}else if(c_id == 10){
				channelTonji('<?php echo $base_url.'report/bossZhouReport'; ?>');
			}
		}
