...
writeLog("switchNotVIPRedis start --------------");
...
function switchRedisProxys ($config){
	if($config){
		...
		$redis->set ( 'notvip_iplist', json_encode($ip_list) );
		$redis->close ();
		$num = count($ip_list);
		writeLog("switchNotVIPRedis success $num:".json_encode($ip_list));
		return $num;
	}else{
		writeLog("switchNotVIPRedis error: config=$config");
	}
	return 0;
}
