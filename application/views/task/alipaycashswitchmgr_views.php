...
<html lang="en">
								...
								<div class="col-xs-12 col-sm-12 widget-container-span">
									<div class="widget-box">
                                        ...
                                        <div class="widget-body">
											<div class="widget-main" style="padding: 0;">
												<button onclick="addAlipayConfig();" class="btn btn-danger" style="border: none;margin-bottom: 10px;">添加新支付宝</button>												
													...
													<table id="sample-table-2"
														...
														<?php if(!empty($config_list)){ foreach ($config_list as $v){ ?>
															<tr>
																...
																<td class="td_user_id" _data="<?php echo $v['app_id']; ?>"><a href="<?php echo site_url('task/alipaycashswtichmgr/configInfo').'?app_id='.$v['app_id']; ?>"><?php echo $v['app_id']; ?></a></td>
																<td><?php echo $v['email']; ?></td>
																<td><?php echo $v['pc_ip']; ?></td>
																<td><?php echo $v['update_admin']; ?></td>
																<td><?php echo $v['update_ip']; ?></td>
																...
																<td style="width:60px;">
																	<a onclick="closeOne('<?php echo $v['app_id']; ?>');" href="javascript:;">禁用</a>|<br/>
																	<a onclick="deleteOne('<?php echo $v['app_id']; ?>');" href="javascript:;">删除</a>
																</td>
																<?php } ?>
															</tr>
														<?php } ?>
	...
	function addAlipayConfig()
	{
		location.href = '<?php echo site_url('task/alipaycashswtichmgr/addAlipayConfig'); ?>';
		return true;
	}

	function deleteOne(app_id)
	{
		if (!confirm('确定删除吗？'))
		{
			return;
		...
		location.href = '<?php echo site_url('task/alipaycashswtichmgr/delete') . "?app_id="; ?>' + app_id;
		return;
	}
