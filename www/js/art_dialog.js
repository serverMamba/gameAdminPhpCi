	...
	_load = jq ?
		...
		function(url, fn, cache){
			jq.ajax({
				...
				cache: cache,
				error:function(){fn && fn('<p><h2 style="color:red;">404 Page Not Found</h2></p><br /><p><h3>The page you requested was not found.</h3></p>');}
			});
		} :
		// 仅提供基本AJAX支持
		function(url, fn, cache){
...
// @return {Object} 对话框操控接口
$.fn.dialog.confirm = function(title, content, yes, no){
	return $.dialog({
			id: _name + 'Confirm',
			icon: 'confirm',
			fixed: true,
...
art.dialog.defaults.title 		= '网站提示';
art.dialog.defaults.effect 		= false;
