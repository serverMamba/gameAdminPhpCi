...
sql = "SELECT SUM(cash_money) AS cash_money,SUM(cash_money - real_cash_money) AS choushui_money, sum(alifee) as alifee, channel_id FROM %s WHERE status = 1 AND update_time >= %s AND update_time <= %s GROUP BY channel_id"%(table_name, today_time,today_time_end)
...
table_name = "smc_cash_order"
sql = "SELECT SUM(cash_money) AS cash_money1,SUM(cash_money - real_cash_money) AS choushui_money1, sum(alifee) as alifee1, channel_id FROM %s WHERE status = 1 AND update_time >= %s AND update_time <= %s GROUP BY channel_id"%(table_name, today_time,today_time_end)
