...
<!--                                                       style="margin-left:5px;height:34px;width:160px;"/>-->
                                        ...
                                        <button onclick="deleteblack('<?php echo site_url('no3/cashOrder/clearBlackAlipayAccount'); ?>');"
                                                class="btn btn-danger" style="border: none;float:right;">清空支付宝黑名单
                                        </button>
                                    ...
                                    <div class="widget-body">
                                        <div class="widget-main" style="padding: 0;">
                                            <form action="" method="post" name="of" id="of">
                                                <button onclick="batchFinish('<?php echo site_url('no3/cashOrder/batchFinish'); ?>');"
                                                        class="btn btn-danger"
                                                        style="border: none;margin-bottom: 10px;">批量处理完成
                                                </button>
                                                <button onclick="batchProcessAgain('<?php echo site_url('no3/cashOrder/batchProcessAgain'); ?>');"
                                                        class="btn btn-danger"
                                                        style="border: none;margin-bottom: 10px;">批量重新处理
                                                </button>
                                                <button onclick="batchFinish('<?php echo site_url('no3/cashOrder/batchSuccess'); ?>');"
                                                        class="btn btn-danger"
                                                        style="border: none;margin-bottom: 10px;">批量处理成功
                                                </button>
                                                <?php if ('10' == $tip_msg) { ?><b><font color="red">支付宝提现异常停止，请通知技术人员重启提现进程！！！</font></b><?php } else if ('00' == $tip_msg) { ?>
                                                    <b><font color="blue">支付宝提现已经停止，请通知技术人员查看或开始人工审核</font></b><?php } else { ?>
                                                    <b><font color="green">支付宝提现运行中...</font></b><?php } ?>
...
<!--                                                        <th>操作员</th>-->
                                                    ...
                                                    <tbody id="tbody">
                                                    <?php if (!empty($cashOrderList)) {
                                                        foreach ($cashOrderList as $v) { ?>
                                                            <tr>
                                                                <td><input type="checkbox" class="line_check_box"
                                                                           name="select_cash_ids[]"
                                                                           value="<?php echo $v['order_sn']; ?>"/></td>
                                                                <td class="td_user_id"
                                                                    _data="<?php echo $v['user_id']; ?>">
                                                                ...
                                                                <td>
                                                                    ...
                                                                    <b><font size="1">今日充值:</font></b><br/><font <?php if (intval($v['todayBuy']) <= 0) { ?> color="red"<?php } ?>><b><?php echo $v['todayBuy']; ?>
                                                                            元</b></font><br/>
                                                                </td>
                                                                <td>
                                                                    <?php echo $v['cash_total_money']; ?>元<br/><br/>
                                                                    <b><font size="1">今日提现:</font></b><br/><font <?php if (intval($v['todayCash']) >= 10000) { ?> color="red"<?php } ?>><b><?php echo $v['todayCash']; ?>
                                                                            元</b></font><br/>
                                                                </td>
                                                                ...
                                                                <td>
                                                                    <font color="<?php echo $statusTextColor[$v['status']]; ?>"><?php echo $statusShowText[$v['status']];
                                                                        ...
                                                                        ?></font>
                                                                    <p>提现总额<?php echo $v['cash_total_money']; ?>元<br/>本次提现<?php echo $v['cut_money']; ?>
                                                                        元</p>
                                                                </td>
                                                                <td><?php
                                                                    if ($v['update_time']) {
                                                                ...
                                                                <td>
                                                                    <?php if ($v['status'] == CASH_ORDER_STATUS_NEW || $v['status'] == CASH_ORDER_STATUS_DEALING || $v['status'] == CASH_ORDER_STATUS_WAIT_REVIEW) { ?>
                                                                        <a onclick="refund('<?php echo $v['order_sn']; ?>');"
                                                                           href="javascript:;">退款</a>
                                                                    <?php } ?>
                                                                    <?php if ($v['status'] == CASH_ORDER_STATUS_WAIT_REVIEW || $v['status'] == CASH_ORDER_STATUS_UNKNOWN) { ?>
                                                                        |
                                                                        <a onclick="processAgain('<?php echo site_url('no3/cashOrder/processAgain') . '?id=' . $v['order_sn']; ?>');"
                                                                           href="javascript:;">重新处理</a>
                                                                    <?php } ?>
...
<div class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel"
     aria-hidden="true" id="chat_modal">
            ...
            <div class="modal-body">
                <div style="margin-top: 10px;width: 100%;">
                    ...
                    <button style="margin-top: 5px;float: right;"
                            onclick="ref('<?php echo site_url('no3/cashOrder/refund'); ?>');" id="reply_btn"
                            class="btn btn-success">退款
                    </button>
                </div>
            </div>
        </div>
