...
class Infojindoubianhua_gs extends MY_Controller {
	public function __construct() {
		parent::__construct ( false, false );
	if (! $this->Common_model->isLogin ()) {
			redirect ( 'no3/login' );
		}
		if (! $this->Common_model->isPriv ( 'kfgl_wjjdbh_gs' )) {
			redirect ( 'no3/index' );
		}
		$this->load->helper ( 'other' );
	}
	public function get_jindoubianhua_data() {
		$this->load->model ( 'jindoubianhua_gs_model' );
		
		$mystarttime = $this->input->get_post ( 'mystarttime' );
		$myendtime = $this->input->get_post ( 'myendtime' );
		$userid = $this->input->get_post ( 'userid' );
		
		$res = $this->jindoubianhua_gs_model->get_jindoubianhua_his ( $userid, $mystarttime, $myendtime );
	...
	public function index() {
		...
		$this->load->view ( 'no3/infojindoubianhuaview_gs', $data );
	}
