 ...
 * 支付相关
 */
class castkami_model extends MY_Model {
    var $db = null;
    ...
    public function generatedo($action, $price, $exp, $count) {
        $tablename = "CASINOCARDSYSTEM";
        $CI = &get_instance();
        $db = $CI->load->database('globalinfo', true);

        $createtime = date('Y-m-d H:i:s', time());
        $mac = "00:26:b9:34:ea:59";
        $count = $count + 0;
        $rr = 0;
        for ($i = 0; $i < $count; $i++) {
            $cardno = mt_rand(1000000000,9999999999)."";
            $password = crc32($mac.$price.$exp."jdsvkjprekgzeriiogwuigfw".$cardno);
    ...
    public function exportData($action,$goodtype,$myendtime,$mystarttime,$status,$userid,$beginindex) {
        ...
        $db = $CI->load->database('gamebuy', true);
         ...
         $where = "where ordertime  >= '$mystarttime' and  ordertime <= '$myendtime'";
    ...
    public function get_kami_msg($action,$goodtype,$myendtime,$mystarttime,$status,$userid,$beginindex){
        $CI = &get_instance();
        $db = $CI->load->database('globalinfo', true);
        // 判断是否合法的时间
        if (!isDate($mystarttime))
        ...
        if (!isDate($myendtime))
        {
         ...
         $where = "where createtime  >= '$mystarttime' and  createtime <= '$myendtime'";
      /*  
        if (!empty($userid)) {
            $where =  $where ." and userid = $userid" ;
         }
        ...
        */
        $sql =  " select * from  CASINOCARDSYSTEM $where ";
