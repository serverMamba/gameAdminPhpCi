...
class Infohf extends MY_Controller {
...
public function get_hf_data() {
       ...
       */
       ...
       $res = $this->hf_model->get_hf_his($userid,$eventid,$mystarttime,$myendtime,$account,$beginindex);
	...
	public function index() {
        ...
        if (file_exists($myfilename)) {
          ...
          // {
           // $gamecodehuang = array_merge_recursive($this->config->item('gamelisthuang'),$this->config->item('gamelist'));
          // }
           $data =array(
               "systemconfig" => $this->configs->get_navmenu(),
               "menucheck" => $menucheck,
               "gamelist" => $this->config->item('eventlist'),
               "gamelists" => $gamecodehuang,
               "eventlists" => $this->config->item('eventlists'),
               "message" =>array("username"=> $usernamezz,"mail"=>"aaa"),
               "choose" => array("father"=>"客服管理","child"=>"玩家话费赛记录"),
               "header1" => array("father"=>"客服管理","child"=>"玩家话费赛记录"),
               "header2" => array("father"=>"玩家话费赛记录","child"=>"玩家话费赛记录 "),
               "header3" => array("father"=>"玩家PK赛记录后台创建于2014年7月2日","child"=>" 游戏运营从2014年7月15日开始 (v1.0) "),
          );
           $this->load->view('no3/infohfview',$data);
	}
