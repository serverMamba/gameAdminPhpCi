...
writeLog("switchVIP2Redis start --------------");
...
function switchRedisProxys ($config){
	if($config){
		...
		$redis->set ( 'vip2_iplist', json_encode($ip_list) );
		$redis->close ();
		$num = count($ip_list);
		writeLog("switchVIP2Redis success $num:".json_encode($ip_list));
		return $num;
	}else{
		writeLog("switchVIP2Redis error: config=$config");
	}
	return 0;
}
