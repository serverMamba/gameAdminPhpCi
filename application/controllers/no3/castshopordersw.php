...
class Castshopordersw extends MY_Controller {
    ...
    public function  exportData(){
         $this->load->model('castshopordersw_model');
          $action = $this->input->get_post('action');
         ...
         $res = $this->castshopordersw_model->exportData($action,$goodtype,$myendtime,$mystarttime,$status,$userid,$beginindex);
       }
        ...
        public function  save_config_data(){
           ...
           $res = $this->castshoporderws_model->save_config_data($data1,$data2);
           echo "配置已经生成！";
        }
       ...
       public function get_castshoporder_data(){
            ...
            $userid = $this->input->get_post('userid');
            $beginindex = $this->input->get_post('beginindex');
            $res = $this->castshopordersw_model->get_shoporder_msg($action,$goodtype,$myendtime,$mystarttime,$status,$userid,$beginindex);
	...
	public function index() {
        ...
        if (file_exists($myfilename)) {
           ...
           $data =array(
               ...
               "choose" => array("father"=>"运营管理","child"=>"实物订单管理(实物)"),
               "header1" => array("father"=>"运营管理","child"=>"实物订单管理(实物)"),
               "header2" => array("father"=>"实物订单管理(实物)","child"=>"管理用户商品请求订单。 "),
               "header3" => array("father"=>"实物订单管理(实物)后台创建于2014年8月3日","child"=>"实物订单管理(实物)从2014年8月5日开始 (v1.0) "),
          );
           $this->load->view('no3/castshoporderswview',$data);
	}
